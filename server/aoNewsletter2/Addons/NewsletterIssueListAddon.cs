    ...
    public class NewsletterIssueListAddon : AddonBaseClass {
        ...
        internal static void processForm(CPBaseClass cp, int newsletterId) {
            try {
                ...
                if (button == Constants.buttonBack) {
                    string detailLink = cp.AdminUI.GetPortalFeatureLink(Constants.guidPortalShare, Constants.guidPortalFeatureNewsletterDetail) + $"&{Constants.rnNewsletterId}={newsletterId}";
        ...
        internal static string getForm(CPBaseClass cp, int newsletterId) {
            try {
                ...
                //
                int rowPtr = 0;
                string detailLinkBase = cp.AdminUI.GetPortalFeatureLink(Constants.guidPortalShare, Constants.guidPortalFeatureNewsletterIssueDetail);
                using (var csList = cp.CSNew()) {
                    if (csList.OpenSQL(sql)) {
                        ...
                        do {
                            ...
                            string detailLink = $"{detailLinkBase}&{Constants.rnIssueId}={issueId}&{Constants.rnNewsletterId}={newsletterId}";
                            issueName = $"<a href=\"{detailLink}\">{issueName}</a>";
